...
class DCShadowNet(object) :
    ...
    def load(self, dir, step):
        params = torch.load(os.path.join('/content/gdrive/MyDrive/DC-ShadowNet-Hard-and-Soft-Shadow-Removal', dir, self.dataset + '_params_%07d.pt' % step))
    ...
    def test(self):
        model_list = glob(os.path.join('/content/gdrive/MyDrive/DC-ShadowNet-Hard-and-Soft-Shadow-Removal', self.result_dir, self.dataset, 'model', '*.pt'))
        if not len(model_list) == 0:
            model_list.sort()
            print('model_list',model_list)
            iter = int(model_list[-1].split('_')[-1].split('.')[0])
            for i in range(-1,0,1):
                self.load(os.path.join('/content/gdrive/MyDrive/DC-ShadowNet-Hard-and-Soft-Shadow-Removal', self.result_dir, self.dataset, 'model'), iter)
                print(" [*] Load SUCCESS")
