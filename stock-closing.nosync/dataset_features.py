...
DATA_FILE_DIR = '/content/train.csv'
DROP_FEATURES = [
    # 'far_price',
    # 'near_price',
    'row_id',
...
class StockDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data_filepath: str, window_size: int = 10) -> None:
        ...
        self.data = data.sort_values(by=['stock_id', 'time_id'])
        self.targets = self.data.groupby('stock_id')['target'].shift(-1).dropna().values
    ...
    def __getitem__(self, idx) -> tuple[torch.Tensor, torch.Tensor]:
        stock_data = self.data.iloc[idx - self.window_size + 1 : idx + 1]
        # Pad with zeros if not enough data
        if len(stock_data) < self.window_size:
            padding = np.zeros((self.window_size - len(stock_data), self.data.shape[1]))
            stock_data = pd.concat([pd.DataFrame(padding, columns=self.data.columns), stock_data], ignore_index=True)

        window = stock_data.values
